...
@section('content')
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 hover:shadow-md hover:border-emerald-300 transition-all">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-500 mb-1">Media & Berita Aktif</p>
                    <h3 class="text-3xl font-bold text-slate-900">{{ $totalActiveMedias }}</h3>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 hover:shadow-md hover:border-rose-300 transition-all">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-500 mb-1">Media & Berita Non Aktif</p>
                    <h3 class="text-3xl font-bold text-slate-900">{{ $totalInactiveMedias }}</h3>
                </div>
    ...
    <section class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-8 border-b border-slate-200">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    <h2 class="text-lg font-bold text-slate-900">Daftar Media & Berita</h2>
                    <p class="text-sm text-slate-500 mt-1">Kelola media & berita yang Anda miliki.</p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('medias.create') }}" class="bg-green-500 hover:bg-green-400 text-white font-semibold px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center space-x-2">
        ...
        <div class="bg-white rounded-lg shadow-sm px-8 py-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                {{-- Search Input --}}
                <div class="flex-1">
                    <form action="{{ route('medias.index') }}" method="GET" class="w-full">
                        <input 
                            type="text" 
                            name="search"
